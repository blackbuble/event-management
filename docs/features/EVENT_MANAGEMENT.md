...
- ~~**Legacy booking controller**~~ — removed (was unreferenced dead code calling non-existent `BookingService` methods; the live flow is `Web\BookingController` + `BookingService` + Inertia `Bookings/Pay|Show`). The now-unused `User::reservations()` / `Event::reservations()` relations were dropped with it. The `BookingReservation` model/table remain inert for a possible future reservation flow.
